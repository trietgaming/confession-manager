...
import { HandleAction, PendingChanges, PostTemplateSettings } from "types";
...
const Posting: Component = () => {
  ...
  const [postSettings, setPostSettings] = createStore<PostTemplateSettings>({});
  const [isPosting, setPosting] = createSignal(false);
  const [newPostLink, setNewPostLink] = createSignal("");
  ...
  const handlePost = () => {
    ...
    FB.api(
      ...
      "post",
      {
        ...
        link:
          postSettings.embed === confesisonForm.responderUri
            ? `${postSettings.embed}?post_by=confession-manager`
            : postSettings.embed,
      },
  ...
  return (
    <>
      <Modal
        ...
        handleSubmit={() => {
          window.open(newPostLink(), "_blank", undefined);
          setNewPostLink("");
        }}
        isShow={!!newPostLink().length}
        handleClose={() => setNewPostLink("")}
      >
        Bài viết của bạn đã được đăng lên trang thành công.
        <br />
        Link bài viết:{" "}
        <a class="text-blue-500" href={newPostLink()} target="_blank">
          {newPostLink()}
        </a>
        <br />
      ...
      <div class="flex justify-between mt-4 mx-2 relative">
        ...
        <div class="relative flex flex-col items-center space-y-4 mb-4 max-h-[90vh] overflow-auto z-0 px-16">
          ...
          >
            ...
            <div ref={postContainer}>
              ...
              >
                ...
                {postSettings.header?.length &&
                  (postSettings.dividerEnabled ? (
                    <div>{postSettings.divider || DEFAULT_DIVIDER}</div>
                  ) : (
                    <br />
                  ))}
              </div>
              ...
              <div>
                {(postSettings.footer?.length ||
                  (postSettings.formLinkEnabled &&
                    !postSettings.formLinkAtFooterDisabled)) &&
                  (postSettings.dividerEnabled ? (
                    <div>{postSettings.divider || DEFAULT_DIVIDER}</div>
                  ) : (
                    <br />
        ...
        <div class="flex flex-col bg-white px-8 mr-2 rounded-lg shadow-md min-w-[20%] max-h-[90vh] space-y-3 pb-4 overflow-auto">
          <PostingContext.Provider
            value={{ storeSetter: setPostSettings, store: postSettings }}
          >
            <PostSetting context={PostingContext} />
          </PostingContext.Provider>
        </div>
      </div>
    </>
