...
const Notification: Component<{ payload: MessagePayload }> = (props) => {
  const notification: NotificationPayload =
    props.payload.notification || props.payload.data?.eventType === "RESPONSES"
      ? {
  ...
  return (
    <>
      <div class="flex px-2 py-4 space-x-4 items-center hover:bg-gray-200 rounded-lg my-2">
        <img src={notification.icon} alt="Icon" class="w-10 h-10" />
        <div class="text-left">
...
const NotificationBell: Component = () => {
  ...
  createEffect(() => {
    ...
    setCachedNotifications((prev) => [...prev, ...currentPendingNotification]);
  });
  ...
  const handleBellClick = async () => {
    ...
    userResourceDatabase.setItem(LOCAL_KEY_PENDING_NOTIFICATIONS, []);
    setPendingNotification([]);
  };
